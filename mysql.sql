...
-- Query 9
SELECT b.Title, ba.Author_Name, JULIANDAY(bl.Returned_Date)-JULIANDAY(bl.Date_Out)+1 AS Days_Borrowed, JULIANDAY(bl.Returned_Date)-JULIANDAY(bl.Due_Date) AS Days_Overdue
FROM BOOK AS b, BOOK_AUTHORS AS ba, BOOK_LOANS AS bl, BORROWER AS bw
WHERE bw.Name='Ethan Martinez' AND b.Book_Id=ba.Book_Id AND b.Book_Id=bl.Book_Id AND bl.Card_No=bw.Card_No
